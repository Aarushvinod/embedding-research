...
## Interpretability experiments (pre-registered; on the trained students, no retraining)

Three experiments, each with its reading rule written BEFORE the numbers exist. Scope: the 10 study
languages only (no zero-shot probing of untrained languages). Scripts: `byte_embed/interp_*.py`
(shared plumbing in `interp_common.py`; every script has a torch-free `--selftest`); dispatch:
`slurm/submit_interp.sh` (one job per model, main grid only); outputs:
`results/interp_<x>_part_<model>.json`, merged + tabulated by `--merge`; figures:
`python gen_interp_figures.py`. Every intervention is paired against **the same model's own
unedited pass through the same loader** (`none` in exp 3, `NN` in exp 4), not against the stored
training-time file — so no delta can absorb loader drift (a different transformers build, a
different checkpoint resolution). That unedited pass is also compared cell-by-cell to the stored
results as the **loader identity check** (tolerance 0.005 nDCG@10); `--merge` prints PASS/WARN and
`slurm/status.sh` flags a WARN. All significance is `stats.paired_bootstrap` over query ids.

| # | script | claim under test | what is done | pre-registered reading rule |
|---|---|---|---|---|
| 3 | `interp_english` | an English-centric model represents non-English inputs partly through English-associated features in its middle layers; byte depends on that less | per-position residual states at 4 depths on FLORES (10-way parallel, so the direction is language, not topic; `</s>` included, because the hook edits it too); a rank-1 LEACE eraser for "English vs not" per depth plus one per other language and one random direction per depth (same whitened metric); an English-vs-rest probe per depth (latent-English curve; picks the block where non-English positions are most English-like); a forward hook erases the direction inside the encoder and the remaining blocks, pooler and projection run on the edited activations; an UNEDITED pass is the baseline and the loader identity check; 10x10 Belebele matrix "effect on L of erasing M" at the chosen block (English/random at all 4 depths); unedited + English + random on the full 20k battery; FLORES embeddings raw vs English-erased for the t-SNE figure. Treatment and control are averaged over the SAME population (languages other than English and other than the erased one) and every mean carries a paired bootstrap CI; the byte-vs-subword contrast is reported both at each model's own chosen block and at MATCHED depth fractions | English column hurts the other languages beyond the random and own-direction controls, more for subword than byte => English is load-bearing and byte depends on it less; hurts only English itself in both => the English-centric story is wrong for both; hurts both equally => inherited from the teacher. Limits: one linear direction at one depth; Belebele's passages come from FLORES, so the Belebele matrix is not text-disjoint from the fitting set (the parallel design holds content constant across the classes, and the 20k battery — MIRACL/QA passages — is the disjoint confirmation) |
| 4 | `interp_script` | byte's advantage is largest on native non-Latin scripts and may partly reverse when they are romanized | (1) byte−subword on BELEBELE (the same 488 passages in all ten languages: one cell per language, so the bootstrap units are independent and content is held constant), non-Latin vs Latin, contrast bootstrapped over languages; the all-cell pooling is printed alongside but its CI is optimistic because one language contributes several cells; the five CROSS-LINGUAL cells (CIRAL en→ha, AfriQA X→en) are held out of both, since they are Latin-labelled but are not native-script monolingual retrieval; (2) te/bn/am/ar/zh on the cached pools + Belebele under NN (native, re-encoded through THIS loader — the baseline and the identity check), RR (queries+passages romanized) and RN (romanized queries vs native passages, counted so a query-set mismatch cannot silently re-measure NN); Buckwalter (ar, lossless) and tone-marked pinyin (zh) as second schemes; the transform is applied AFTER the model's 512-character truncation, so romanization does not also delete two thirds of the passage; (3) FLORES native↔romanized cosine and centroid distance to the Latin cluster | larger byte advantage on non-Latin cells AND a larger byte drop under RR/RN (difference-in-differences with its own bootstrap interval) => better for native scripts, paid for on romanized input; no differential drop => the advantage comes free. Only the difference is interpretable: romanization destroys information for everyone |
| 5 | `interp_segment` (byte only) | byte models develop segmentation on their own, and different languages develop their own rather than one tokenizer-imposed structure | per-byte-position probes at every layer for tok_end / interior (in-run sub-word boundary vs in-run non-boundary) / word_end / count-matched random, all 10 languages, split BY SENTENCE with bootstrap intervals; surface-statistics baseline (one-hot ±4-byte window, no model); identical probes on the untouched pretrained ByT5; cross-lingual transfer of the interior probe at the peak layer (the layer where boundary signal is most active, chosen by mean test accuracy — the same rule for every model) for all 100 pairs, scored as AUC(A→B)/AUC(B→B), with the RAW AUCs reported alongside because the ratio's denominator is the quantity the layer was chosen to maximise; the pretrained encoder is reported both at that layer and at its OWN peak, so "what distillation added" is not an argmax compared against an imposed layer — threshold-free, because each language is standardized with its own statistics and a hard-label accuracy would read "no transfer" whenever only the probe's intercept failed to carry — plus a joint probe | within-language accuracy must beat the surface baseline and random; trained > pretrained = distillation added some; transfer low even within a script (en→sw, sw→rw) => language-specific segmentation (the claim); high within / low across scripts => script-specific; high everywhere => one shared feature. Cross-script cells are expected low on byte-overlap grounds alone |

Dropped after the first round (results in git history): parameter accounting (a setup table, not an
experiment), alignment/uniformity, output-space language erasure (settled: language identity is not
used by retrieval in either architecture), the FLORES script grid, steering.

Operational contract: part files carry a `schema` integer; `run_one` recomputes when it does not
match and `--merge` SKIPS a stale file with a warning rather than mixing two definitions into one
table. Writes are atomic (tmp + replace) and an unreadable part file is treated as absent, because
every job is `--requeue`d on a preemptible partition. Jobs also self-requeue five minutes before the
wall clock (`--signal=B:USR1@300` + `scontrol requeue`, as the training jobs do), since `--requeue`
alone does not cover a TIMEOUT; each script resumes from its part file, so a wall-clock kill costs
